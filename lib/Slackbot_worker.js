...
module.exports = function(botkit, config) {
    ...
    bot.sendWebhook = function(options, cb) {
        if (!bot.config.incoming_webhook || !bot.config.incoming_webhook.url) {
            botkit.debug('sendWebhook CANNOT SEND WEBHOOK!!');
            return cb && cb('No webhook url specified');
        }

        const url = bot.config.incoming_webhook.url;
        console.log(`\nPOST ${url}\n`, url, options);

        request.post(url, function(err, res, body) {
            if (err) {
                botkit.debug('Slack sendWebhook ERROR', err);
                return cb && cb(err);
            }
            botkit.debug('Slack sendWebhook SUCCESS', body);
            cb && cb(null, body);
