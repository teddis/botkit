...
function Slackbot(configuration) {
    ...
    slack_botkit.createWebhookEndpoints = function (webserver, authenticationTokens) {
        ...
        webserver.post('/slack/receive', function (req, res) {
            ...
            //slack_botkit.log("webhook received: " + JSON.stringify(req.body));

            // is this an interactive message callback?
            if (req.body.payload) {
                slack_botkit.log("got payload:");
                ...
                slack_botkit.findTeamById(message.team.id, function (err, team) {
                    if (err || !team) {
                        slack_botkit.log.error("interactive err: " + err);
                        ...
                        var result = slack_botkit.trigger('interactive_message_callback', [bot, message]);
            ...
            } else if (req.body.type && req.body.type == "event_callback") {
                // TODO:
                var message = {};
                for (key in req.body) {
                    message[key] = req.body[key];
                ...
                res.status(200);
                res.send('');
                slack_botkit.trigger('event_callback', [bot, message]);
            } else if (req.body.type && req.body.type == "url_verification") {
                slack_botkit.debug(">>> url_verification: " + JSON.stringify(req.body));
    ...
    slack_botkit.setWebserver = function (webserver) {
        slack_botkit.webserver = webserver;
    };
    ...
    slack_botkit.createOauthEndpoints = function (webserver, callback) {
        ...
        webserver.get('/oauth', function (req, res) {
            ...
            slack_botkit.log('** /oauth called, req: ' + JSON.stringify(req.query));

            var code = req.query.code;
            var clientId = req.query.state;

            slack_botkit.config.getClientSecret(clientId, (err, clientSecret) => {
                if (err) {
                    res.status(500).send("invalid_client_id");
                    return;
                }
                slack_botkit.log("clientSecret: " + clientSecret);
                    ...
                    if (err) {
                        ...
                        auth_test({token: auth.access_token}, function (err, identity) {
                            ...
                            if (err) {
                                ...
                                var channelId = auth.incoming_webhook.channel_id;
                                ...
                                slack_botkit.storage.teams.getByTeamClientChannelId(identity.team_id,
                                    clientId, channelId,
                                    function (err, team) {

                                        slack_botkit.debug("\nGot team: " + JSON.stringify(team));
                                        ...
                                        if (auth.incoming_webhook) {
                                            ...
                                            auth.incoming_webhook.clientId = clientId;
