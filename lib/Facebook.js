    ...
    facebook_botkit.defineBot(function (botkit, config) {
        ...
        bot.send = function (message, cb) {
            ...
            if (typeof(message.channel) == 'string' && message.channel.match(/\+\d+\(\d\d\d\)\d\d\d\-\d\d\d\d/)) {
                msg.recipient = {phone_number: message.channel};
            } else {
                msg.recipient = {id: message.channel};
            }

            if (message.sender_action) {
                msg.sender_action = message.sender_action;
            }

            if (message.text || message.attachment || message.quick_replies) {
                msg.message = {};
            }
            ...
            if (message.attachment) {
                msg.message.attachment = message.attachment;
            }

            if (message.quick_replies) {
                msg.message.quick_replies = message.quick_replies;
            }

            var url = 'https://graph.facebook.com/v' + FacebookVersion
                + '/me/messages?access_token=' + bot.config.access_token;
            ...
            request({
                ...
                if (err) {
                    botkit.debug('Facebook send ERROR', err);
                    return cb && cb(err);
                }

                if (body.error) {
                    botkit.debug('Facebook send FAILURE', body.error);
                    return cb && cb(body.error.message);
                }

                botkit.debug('Facebook send SUCCESS', body);
                cb && cb(null, body);
            });
        };

        bot.sendThread = function (message, cb) {
            var url = 'https://graph.facebook.com/v' + FacebookVersion
                + '/me/thread_settings?access_token=' + bot.config.access_token;
            ...
            request({
                ...
                if (err) {
                    botkit.debug('Facebook send thread ERROR', err);
                    return cb && cb(err);
                }

                if (body.error) {
                    botkit.debug('Facebook send thread FAILURE', body.error);
                    return cb && cb(body.error.message);
                }

                botkit.debug('Facebook send thread SUCCESS', body);
                cb && cb(null, body);
            });
        };
